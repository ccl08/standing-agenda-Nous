...
// notionSync() is chained at the end of dailySync() — there is
// no separate trigger for this function.
// ============================================================
...
function notionSync() {
  ...
  var posts = nsLoadPosts_();
  var corr  = nsLoadCorrections_();
  var lw    = nsLoadLastWritten_();

  Logger.log('Amplitude rows : ' + amp.length);
  Logger.log('Posts in window: ' + posts.length);
  Logger.log('Corrections    : ' + Object.keys(corr).length);
  Logger.log('LastWritten    : ' + Object.keys(lw.cache).length + ' cached entries');
  ...
  var updated = 0, skipped = 0, unchanged = 0, errors = 0;
  var logs    = [];
  var runTime = new Date();
  ...
  posts.forEach(function(post) {
    try {
      ...
      if (m.lp === 0) {
        ...
        if (urlKey && urlKey !== key) {
          var m2 = nsSumWindow_(amp, urlKey, dateWindow);
          if (m2.lp > 0) { m = m2; resolvedKey = urlKey; matchType = 'url'; }
        }
      }
      ...
      if (m.lp === 0) {
        var fb = nsSimilarityFallback_(key, amp, dateWindow);
        if (fb) { m = fb.metrics; resolvedKey = fb.key; simScore = fb.sim; matchType = 'sim'; }
      }
      ...
      // Delta detection — skip Notion write if values haven't changed since last run
      var cached = lw.cache[post.pageId];
      if (cached && cached.lp === m.lp && cached.acc === m.acc && cached.del === m.del) {
        unchanged++;
        Logger.log('= ' + post.influencer + ' | unchanged LP=' + m.lp + ' Acc=' + m.acc + ' Del=' + m.del);
        logs.push([runTime, 'unchanged', post.influencer]);
        return;
      }

      nsUpdateNotionPage_(post.pageId, m.lp, m.acc, m.del);
      nsSetLastWritten_(lw, post.pageId, m.lp, m.acc, m.del);
      Logger.log('✓ ' + post.influencer + ' | ' + dateWindow.join('+') +
        ' | LP=' + m.lp + ' Acc=' + m.acc + ' Del=' + m.del + ' [' + resolvedKey + '] (' + matchType + ')');

      var logStatus;
      if (matchType === 'sim') {
        logStatus = '⚠️ SIMILARITY GUESS (score=' + Math.round(simScore * 100) + '%): wrote data from key "' +
          resolvedKey + '" — confirm and add to Corrections sheet. LP=' + m.lp + ' Acc=' + m.acc + ' Del=' + m.del;
      } else if (matchType === 'url') {
        logStatus = '🔗 URL fallback: matched via utm_campaign in ampMatchUrl ("' + resolvedKey +
          '"). LP=' + m.lp + ' Acc=' + m.acc + ' Del=' + m.del;
      } else {
        logStatus = '✅ matched LP=' + m.lp + ' Acc=' + m.acc + ' Del=' + m.del;
      }
      logs.push([runTime, logStatus, post.influencer]);
      updated++;
  ...
  Logger.log('=== Done: updated=' + updated + ' unchanged=' + unchanged + ' skipped=' + skipped + ' errors=' + errors + ' ===');
  nsWriteLog_(logs);
}
...
function nsLoadCorrections_() {
  try {
    ...
    var ws     = null;
    for (var i = 0; i < sheets.length; i++) {
      if (sheets[i].getSheetId() === 0) { ws = sheets[i]; break; }
    }
    if (!ws) ws = sheets[0]; // fallback to first tab if gid 0 not found
    if (!ws) {
      Logger.log('Corrections: spreadsheet has no tabs — continuing without corrections');
      return {};
    }

    var raw = ws.getDataRange().getValues();
    if (raw.length < 2) {
      Logger.log('Corrections: tab is empty or header-only — continuing without corrections');
      return {};
    }
    ...
    for (var i = 1; i < raw.length; i++) {
      ...
      var key  = String(raw[i][1] || '').trim().toLowerCase();
      if (name && key) map[name] = key;
    }
    return map;

  } catch (e) {
    Logger.log('Corrections: could not open spreadsheet — ' + e.message + ' — continuing without corrections');
    return {};
  }
}
...
function nsSimilarityFallback_(key, amp, dateStrs) {
  ...
  if (bestSim >= 0.75 && bestKey) {
    ...
    if (m.lp > 0) return { key: bestKey, metrics: m, sim: bestSim };
  }
  return null;
}
...
// ── LastWritten cache ─────────────────────────────────────────
// Persists the last-written LP/Acc/Del per pageId in a 'LastWritten' tab
// on the Amplitude spreadsheet. Loaded once per notionSync() run.
// testNotionSync() and testTwoDayWindow() never call these functions.

function nsLoadLastWritten_() {
  var ss = SpreadsheetApp.openById(NS_AMPLITUDE_SHEET_ID);
  var ws = ss.getSheetByName('LastWritten');

  if (!ws) {
    ws = ss.insertSheet('LastWritten');
    ws.appendRow(['pageId', 'lp', 'acc', 'del', 'updatedAt']);
    Logger.log('LastWritten tab created — all posts treated as changed on this run');
    return { ws: ws, cache: {} };
  }
  ...
  var cache = {};
  for (var i = 1; i < raw.length; i++) {
    var pageId = String(raw[i][0] || '').trim();
    if (!pageId) continue;
    cache[pageId] = {
      lp:  Number(raw[i][1] || 0),
      acc: Number(raw[i][2] || 0),
      del: Number(raw[i][3] || 0),
      row: i + 1  // 1-based sheet row index
    };
  }
  return { ws: ws, cache: cache };
}

function nsSetLastWritten_(lw, pageId, lp, acc, del) {
  var now = new Date();
  if (lw.cache[pageId]) {
    lw.ws.getRange(lw.cache[pageId].row, 1, 1, 5).setValues([[pageId, lp, acc, del, now]]);
  } else {
    lw.ws.appendRow([pageId, lp, acc, del, now]);
    lw.cache[pageId] = { lp: lp, acc: acc, del: del, row: lw.ws.getLastRow() };
  }
  lw.cache[pageId].lp  = lp;
  lw.cache[pageId].acc = acc;
  lw.cache[pageId].del = del;
}
